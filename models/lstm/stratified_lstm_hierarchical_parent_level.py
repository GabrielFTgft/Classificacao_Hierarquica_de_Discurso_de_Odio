...
# Suprimir warnings de UndefinedMetricWarning
warnings.filterwarnings('ignore', category=UndefinedMetricWarning)

def estratificacao(dados):

    dados = dados.sample(frac=1, random_state=42)

    quantidade_por_classe = dados.iloc[:, 1:].sum().sort_values()

    #print("Quantidade por classe: ", quantidade_por_classe)

    treino = pd.DataFrame(columns=dados.columns)
    teste = pd.DataFrame(columns=dados.columns)

    for classe, _ in quantidade_por_classe.items():
        amostras_classe = dados[dados[classe] == 1] 
        tamanho_teste =  int(0.1 * len(amostras_classe))  

        amostras_teste = amostras_classe.sample(n=tamanho_teste, random_state=42)
        dados = dados.loc[~dados.index.isin(amostras_teste.index)]
        
        teste = pd.concat([teste, amostras_teste])

        amostras_treino = amostras_classe.drop(amostras_teste.index)
        dados = dados.loc[~dados.index.isin(amostras_treino.index)]
        treino = pd.concat([treino, amostras_treino])


    # Exibir as quantidades de cada classe nos conjuntos de treino e teste
    # quantidade_treino = treino.iloc[:, 1:].sum().sort_values()
    # quantidade_teste = teste.iloc[:, 1:].sum().sort_values()

    # print("Distribuição no conjunto de treino:")
    # print(quantidade_treino)

    # print("Distribuição no conjunto de teste:")
    # print(quantidade_teste)

    tamanho_no_hate_speech =  int(0.1 * len(dados))
    amostras_no_hate_speech = dados.sample(n=tamanho_no_hate_speech, random_state=42)
    teste = pd.concat([teste, amostras_no_hate_speech])

    dados = dados.drop(amostras_no_hate_speech.index)
...
dados_treino, dados_teste = estratificacao(dados)
...
sequences_treino = tokenizer.texts_to_sequences(dados_treino['text'])  # Converter os textos de treino para índices
X_train = pad_sequences(sequences_treino, maxlen=max_len)  # Ajustar o comprimento das sequências de treino
y_train = dados_treino.drop(columns=['text'])

# Preparar os dados de teste
sequences_teste = tokenizer.texts_to_sequences(dados_teste['text'])  # Converter os textos de teste para índices
X_test = pad_sequences(sequences_teste, maxlen=max_len)  # Ajustar o comprimento das sequências de teste
y_test = dados_teste.drop(columns=['text'])
...
file_path = Path(__file__).resolve().parent.parent.parent / 'data' / 'preprocessed' / 'stratified_lstm_hierarchical_parent_level_predictions.txt'
